...
public class CreditServiceImpl implements CreditService {
    ...
    public Mono<CreditResponse> update(String id, CreditUpdateRequest request) {
        return findEntityById(id)
                .flatMap(existing -> {
                    boolean anyPaid =
                            existing.getInstallments().stream().anyMatch(Installment::isPaid);
                    if (anyPaid) {
                    ...
                    existing.setInstallmentAmount(
                            installmentBaseAmount(request.totalAmount(), request.termMonths()));
                    existing.setInstallments(
                            generateInstallments(request.totalAmount(), request.termMonths()));
    ...
    public Mono<Void> delete(String id) {
        return findEntityById(id)
                .flatMap(credit -> {
                    boolean anyPaid =
                            credit.getInstallments().stream().anyMatch(Installment::isPaid);
                    if (anyPaid) {
                        return Mono.<Credit>error(new InvalidBusinessRuleException(
    ...
    private Mono<Void> validateOnePersonalCreditActive(
            String customerId, CustomerType customerType) {
    ...
    private List<Installment> generateInstallments(
            BigDecimal totalAmount, int termMonths) {
    ...
    private Mono<Integer> validateAndFindTargetInstallment(Credit credit, BigDecimal amount) {
        return Mono.defer(() -> {
            if (credit.getStatus() == CreditStatus.PAID) {
                return Mono.error(new InvalidBusinessRuleException(
                        "El credito ya esta pagado en su totalidad"));
            }
    ...
    private Mono<Credit> applyInstallmentPayment(String creditId, int installmentNumber) {
        ...
        return mongoTemplate
                .findAndModify(
                        query, update, FindAndModifyOptions.options().returnNew(true), Credit.class)
    ...
    private Mono<Payment> recordPayment(
            Credit credit, int installmentNumber, BigDecimal amount, String idempotencyKey) {
